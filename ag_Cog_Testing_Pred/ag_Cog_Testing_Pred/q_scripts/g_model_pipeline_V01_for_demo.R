...
############## Project - Relese - App - Phase - Week - WeekMax Model
if(TRUE){
  if(FALSE){
    dat <- LoadData()
    dat_ProjAppPhase <- GetProjAppPhaseData(dat)
    ...
    Score_ProjAppPhaseWeekMaxData_GLM_RF_GBM_Model(model_DefectCount, 
                                       ...
                                       dev_team_size = 18,
                                       test_team_size = 20)
   # Predicted Defects: 82.7068397435898
   # Number of Weeks Testing will Continue: 4.72004713797748
   Score_ProjAppPhaseWeekMaxData_GLM_RF_GBM_Model(model_DefectCount, 
                                                   model_WeekMax, 
                                                   dat,
                                       phase = "4.1-UNIT TEST",
                                       ...
                                       dev_team_size = 18,
                                       test_team_size = 80)
   # Predicted Defects: 93.7385416666667
...
############## Project - Relese - App - Phase - Week Model
if(FALSE){
  dat <- LoadData()
  dat_ProjAppPhaseWeek <- GetProjAppPhaseWeekData(dat)
  dat_ProjAppPhaseWeek <- FeatureEng_ProjAppPhaseWeekData(dat_ProjAppPhaseWeek)
  # Explore_ProjAppPhaseWeekData(dat_ProjAppPhaseWeek)
  GetProjAppPhaseWeekMaxData(dat)
}
